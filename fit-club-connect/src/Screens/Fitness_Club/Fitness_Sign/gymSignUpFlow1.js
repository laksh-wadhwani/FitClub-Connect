    ...
    const Continue = () => {
        if(isEmailValid(email)){
            ...
            .catch(error => {
                ...
                console.log("Occured Erorr During Club Sign Up: "+error)
            })
            
        }
        else toast.error("Email Address should contain at least 3 letter with @")
    ...
    const Verify = () => {
        ...
        .then(response => {
            if(response.data.message === 'User has been registered successfully'){
                toast.success(response.data.message)
                setTimeout(() => {navigate(`/GymSignUpFlow2/${email}`);},2000)  
            }
            else{
                toast.error(response.data.message)
        ...
        .catch(error => {
            toast.error("Error Occured")
            console.log("Error Occured Druring verifying Club OTP: "+error)
        })    
    }
